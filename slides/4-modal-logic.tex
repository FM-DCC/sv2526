...
\item \alert{a semantic consequence} of a set of formulas $\Gamma$ ($\Gamma \models \phi$) if
  for all models $\ger{M}$,
          points $s$, and
          formula $\gamma \in \Gamma$,
  \\\centering if $\ger{M}, s \models \gamma$ then $\ger{M}, s \models \phi$.
...
%----------------------------------------------------------------------------------
\begin{slide}{Some Modal Logics -- Example I}\label{s:16}
\small
...
%----------------------------------------------------------------------------------
\begin{slide}{Some Modal Logics -- Example II}\label{s:18}
\small
...
%----------------------------------------------------------------------------------
\begin{slide}{Some Modal Logics -- Example III}\label{s:19}
\small
...
\begin{slide}{Some Modal Logics -- Example IV}\label{s:20}
\small
...
%----------------------------------------------------------------------------------
\begin{slide}{Modal logic for processes: Process logic}\label{s:17}
\small
...
\begin{slide}{Modal logic for processes: Process logic}
\small
...
\begin{slide}{Process Logic + regular expressions}
  ...
  \\Often called \alert{dynamic logic}.
  }
...
\alert{If} two states $s, s'$ from two finite models $\ger{M} = \pair{\pair{S, R}, V}$ and $\ger{M}' = \pair{\pair{S', R'}, V'}$ satisfy the same modal formulas,
\\
\alert{then} there is a bisimulation $B \subseteq S \times S'$  such that $s B s'$.
 \end{block}
\end{slide}
...
\small
\begin{itemize}
  \item A modal formula is valid on a frame if it is true under \alert{every valuation} at \alert{every world} (i.e., it cannot be refuted)
  \item The class of frames defined by a modal formula $\phi$ are those where $\phi$ is valid.
  \item Example: $\eventual\eventual p \to \eventual p$ defines \structure{transitivity}:\\
  ~~~~$\ger{F}=\pair{W,R}$ is transitive iff \alert{for all $V$ and $w$},\\
