...
\setLecture{7}{Probabilities: Probabilistic LTS and Statistical Model Checking}
...
\begin{frame}[t]\frametitle{Recall discrete distributions}
  ...
  \begin{itemize}
        ...
        \item \alert{Product distribution}: $(\mu_1 \times\mu_2)\pair{s,t} = \mu_1(s) \times \mu_2(t)$
  \end{itemize}    
  \end{exampleblock}
\end{frame}
...
\begin{frame}[t]\frametitle{Reactive PTS (or Markov Decision Process)}
  \centering
  {\Large \alert{$\alpha: S \to (\Dist(S) \structure{+ 1})^{\structure{N}}$}}

  \bigskip

  \doExercise{Formalise the system below on the right as a function}{
  \centering
  \wrap{\begin{tikzpicture}[aut]
    \node[st,initial above,final]  (s_0) {$A_0$};
    \node[st] (s_1) [above right of=s_0] {$A_1$};
...
\begin{frame}[t]\frametitle{Generative PTS}
  \centering
  {\Large \alert{$\alpha: S \to \Dist(S \structure{\times N}) + 1$}}
  ...
  \doExercise{Now (generative) -- formalise it}{\centering
    ...
    \path[->]
      ...
      (s_1) edge  node {\structure{b[0.7]}}  (s_2)
...
\begin{frame}[t]\frametitle{Generative \alert{Timed} PTS (as in Uppaal)}
  \centering
  \only<1>{{\Large \alert{$\alpha: S \to \Dist(S \times N) \structure{+ 1}$}}}
  \pause
  % {\large \alert{$\alpha: S \to \Dist_{disc}((S\times N) + 1)$}}\\
  % {\Large \alert{$\alpha: S \to \Dist_{\structure{cont}}((S\times (N\structure{+\R_0^{+}}) + 1))$}}
  \only<2->{
    \textcolor{gray}{$\alpha: S \to \Dist(S \times N) + 1$}\\[2mm]
    {\Large \alert{$\alpha: S \to \Dist(S \times N) + 
                            \structure{\Dist_{cont}(\R_0^{+} \times S)}$}}
  }
  % \Pow(X)^{Act} \times (X+1)^{\R^{+}_0}


  \bigskip

  \splittwo{0.46}{0.52}{
  \only<1>{\begin{alertblock}{Before (Gen PTS)}\centering
    \wrap{\begin{tikzpicture}[aut]
    \node[st,initial above,final]  (s_0) {$A_0$};
    \node[st] (s_1) [above right of=s_0] {$A_1$};
    \node[st] (s_2) [below right of=s_0] {$A_2$};
  ...
  \only<2->{
    ...
    \begin{itemize}
      ...
      \item Probabilities both at 
      \begin{itemize}
        \item \alert{discrete transitions} and
        \item \structure{continuous delays}
      \end{itemize}
    \end{itemize}
    \end{block}
  }
  }{
  \doExercise{Now (Timed PTS)}{\centering
    \wrap{\begin{tikzpicture}[aut]
    \node[st,fill=orange!15,initial above,final]  (s_0) {$A_0$};
    \node[st,fill=green!15] (s_1) [above right of=s_0] {$A_1$};
    \node[st,fill=green!15] (s_2) [below right of=s_0] {$A_2$};
    \node[st,fill=orange!15] (s_3) [below right of=s_1,xshift=15mm] {$A_3$};
    \path[->]
      (s_0) edge  node {\alert{b[0.3]}} (s_1) %{\structure{$X_{unif(2,4)}\leq 4$}}  (s_1)
      (s_0) edge  node {\alert{b[0.1]}}  (s_2)
      (s_1) edge  node {\structure{$X_{unif(0,10)}\leq 2$}}  (s_2)
      (s_0) edge[loop left] node {\alert{a[0.6]}}  ()
      (s_1) edge[loop right] node {\structure{$2<X_{unif(0,10)}\leq 5$}}  ()
      (s_1) edge node[yshift=-3pt] {\structure{$5<X_{unif(0,10)}$}}  (s_3)
      (s_3) edge node {\alert{a[1]}} (s_2);
      % (s_2) edge[loop right] node {1}  (s_1);
  \end{tikzpicture}}
...
\item $Tr \subseteq L \times (\cc{C} \alert{\cup \N}) \times Act \times \pow{(C)} \times L$ is the \structure{transition relation}
\vspace*{-1mm}
\begin{align*}
\ell_1\; \trans{g,a,U}\; \ell_2  && or && \ell_1\; \trans{\alert{w},a,U}\; \ell_2
\end{align*}
\vspace*{-1mm}
...
\begin{frame}[t]\frametitle{Again A1,A2,A3: Timed PTS}
  ...
  \splittwo{0.4}{0.58}{
    ...
    \begin{itemize}
      ...
      \pause
      \item \alert<2>{Probability of reaching $A1_1$ within 1?}
      \pause
      \item \alert<3>{Probability of reaching $A1_1$ within 5?}
      \pause
      \item \alert<4>{Probability of reaching $A2_1$ (above) within 5?}
      \pause
      \item \alert<5>{Expected time to reach $A1_{1}$?}
      \pause
      \item \alert<6>{Expected time to reach $A3_{1}$ or $A3_{2}$?}
      \pause
      \item \alert<7>{Expected time to reach $A1_{END}$?}
      \item \alert<7>{Expected time to reach $A2_{END}$?}
      \item \alert<7>{Expected time to reach $A3_{END}$?}
    \end{itemize}
  }
\end{frame}
