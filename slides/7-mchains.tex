...
\setLecture{6}{Probabilities: Probabilistic LTS and Statistical Model Checking}
...
\begin{frame}[t]\frametitle{Generative \alert{Timed} PTS (as in Uppaal)}
  ...
  \doExercise{Now (Timed PTS)}{\centering
    ...
    \node[st] (s_1) [above right of=s_0] {$A_1$};
    \node[st] (s_2) [below right of=s_0] {$A_2$};
    \node[st] (s_3) [below right of=s_1,xshift=15mm] {$A_3$};
    \path[->]
...
\begin{frame}[t]\frametitle{Again A1,A2,A3: Timed PTS}
  ...
  \splittwo{0.4}{0.58}{
    ...
    \begin{itemize}
      ...
      \pause
      \item \alert<5>{Expected time to reach $A3_{1}$ or $A3_{2}$?}
      \pause
      \item \alert<6>{Expected time to reach $A1_{END}$?}
      \item \alert<6>{Expected time to reach $A2_{END}$?}
      \item \alert<6>{Expected time to reach $A3_{END}$?}
    \end{itemize}
  }
\end{frame}
