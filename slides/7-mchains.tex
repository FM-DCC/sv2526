...
\begin{frame}[t]\frametitle{Generative Timed PTS (as in Uppaal)}
  \centering
  ...
  \only<2->{
    ...
    {\Large \alert{$\alpha: S \to (\Dist(S \times N) + 1) \times 
  ...
  \doExercise{Now (Timed PTS)}{\centering
    ...
    \node[st] (s_1) [above right of=s_0] {$A_1$};
    \node[st] (s_2) [below right of=s_0] {$A_2$};
    \node[st] (s_3) [below right of=s_1] {$A_3$};
    \path[->]
      (s_0) edge  node {\structure{$X_{unif(2,4)}\leq 4$}}  (s_1)
      (s_0) edge  node {\alert{b[0.4]}}  (s_2)
      (s_1) edge  node {\alert{b[1]}}  (s_2)
      (s_0) edge[loop left] node {\alert{a[0.6]}}  ()
      (s_1) edge[loop right] node {\structure{$Y_{unif(0,10)}\geq 2$}}  ()
      (s_1) edge node[yshift=-3pt] {\structure{$Y_{unif(0,10)}<2$}}  (s_3);
      % (s_2) edge[loop right] node {1}  (s_1);
  \end{tikzpicture}}
